...
async def handle_purchase(interaction: discord.Interaction, product: dict, bot):
    ...
    if not base_product.get('infinito'):
        deliverables = list(base_product.get('deliverables', []))
    ...
    try:
        ...
        # tenta criar thread privada; se falhar, usa thread pública como fallback
        try:
        ...
        if not base_product.get('infinito') and reserved_item:
            ...
            await update_catalog_display(bot)
        try:
            await interaction.followup.send(embed=create_error_embed("Erro", f"Ocorreu um erro ao criar o pedido: {e}"), ephemeral=True)
        except Exception:
            pass
