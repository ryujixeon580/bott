...
class InMemoryDatabase:
    def __init__(self, filename):
        ...
        self._db_cache = {
            "config": {
                ...
                "pix_cidade": "BRASIL", "quantidade_por_categoria": {}, "admin_commission_percent": 5, "admin_stats": {}, "withdraw_requests": [], "payment_logs": [], "finance": {"total_paid": 0}, "dashboard": {"finance_channel": None, "ranking_channel": None, "logs_channel": None, "finance_message_id": None, "ranking_message_id": None, "logs_message_id": None}, "cargo_aprovador_saque": None
            },
            "products": [], 
            "orders": [], 
            "active_catalogs": []
    ...
    async def get_config(self):
        async with self._lock:
            ...
            cfg.setdefault("quantidade_por_categoria", {})
            ...
            cfg.setdefault("finance", {"total_paid": 0})
            cfg.setdefault("dashboard", {"finance_channel": None, "ranking_channel": None, "logs_channel": None, "finance_message_id": None, "ranking_message_id": None, "logs_message_id": None})
...
class AdminActionsView(discord.ui.View):
    ...
    async def confirm_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        await confirm_order_logic(self.order_id, self.bot, interaction.user.id)
    ...
    async def cancel_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        await interaction.response.defer()
        await cancel_order_logic(self.order_id, "Cancelado via Menu Administrativo", self.bot, interaction.user.id)
...
class PaymentComponentsView(discord.ui.LayoutView):
    ...
    async def handle_admin_gear(self, interaction: discord.Interaction):
        if not await check_admin_permission(interaction, self.bot): return await interaction.response.send_message(view=NoPermissionView(), ephemeral=True)
...
class DashboardSetupModal(discord.ui.Modal, title="Configurar Painéis de Status"):
    def __init__(self, current_config):
        super().__init__()
        dash = current_config.get("dashboard", {})
        self.finance = discord.ui.TextInput(label="Canal Financeiro (ID)", default=str(dash.get("finance_channel") or ""), required=False)
        self.ranking = discord.ui.TextInput(label="Canal Ranking ADM (ID)", default=str(dash.get("ranking_channel") or ""), required=False)
        self.logs = discord.ui.TextInput(label="Canal Logs/Saques (ID)", default=str(dash.get("logs_channel") or ""), required=False)
        self.add_item(self.finance)
        self.add_item(self.ranking)
        self.add_item(self.logs)

    async def on_submit(self, interaction: discord.Interaction):
        current = await db.get_config()
        dash = current.get("dashboard", {})
        dash["finance_channel"] = int(self.finance.value) if self.finance.value.strip() else None
        dash["ranking_channel"] = int(self.ranking.value) if self.ranking.value.strip() else None
        dash["logs_channel"] = int(self.logs.value) if self.logs.value.strip() else None
        dash["finance_message_id"] = None
        dash["ranking_message_id"] = None
        dash["logs_message_id"] = None
        await db.update_config({"dashboard": dash})
        await interaction.response.send_message("✅ Painéis configurados.", ephemeral=True)
        await update_status_embeds(interaction.client)

class CommissionSetupModal(discord.ui.Modal, title="Configurar Comissão ADM"):
    def __init__(self, current_config):
        super().__init__()
        self.percent = discord.ui.TextInput(label="Comissão %", default=str(current_config.get("admin_commission_percent", 5)))
        self.add_item(self.percent)

    async def on_submit(self, interaction: discord.Interaction):
        try:
            percent = float(self.percent.value.replace(",", "."))
            await db.update_config({"admin_commission_percent": percent})
            await interaction.response.send_message(f"✅ Comissão definida em {percent}%.", ephemeral=True)
        except ValueError:
            await interaction.response.send_message("❌ Valor inválido.", ephemeral=True)

class WithdrawRoleSetupModal(discord.ui.Modal, title="Cargo que aprova saque"):
    def __init__(self, current_config):
        super().__init__()
        self.role_id = discord.ui.TextInput(label="ID do cargo aprovador", default=str(current_config.get("cargo_aprovador_saque") or ""))
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message("✅ Cargo aprovador salvo.", ephemeral=True)
        except ValueError:
...
class ConfigSelectionView(discord.ui.View):
    ...
    @discord.ui.button(label="Painéis de Status", style=discord.ButtonStyle.secondary, emoji="📊")
    async def dashboard_config(self, interaction: discord.Interaction, button: discord.ui.Button):
        config_data = await db.get_config()
        await interaction.response.send_modal(DashboardSetupModal(config_data))
    @discord.ui.button(label="Comissão ADM", style=discord.ButtonStyle.secondary, emoji="💸")
    async def commission_config(self, interaction: discord.Interaction, button: discord.ui.Button):
        config_data = await db.get_config()
        await interaction.response.send_modal(CommissionSetupModal(config_data))
    @discord.ui.button(label="Cargo aprova saque", style=discord.ButtonStyle.secondary, emoji="🏦")
    async def withdraw_role_config(self, interaction: discord.Interaction, button: discord.ui.Button):
        config_data = await db.get_config()
        await interaction.response.send_modal(WithdrawRoleSetupModal(config_data))
...
async def _build_status_embeds():
    config = await db.get_config()
    stats = config.get("admin_stats", {})
    withdraws = config.get("withdraw_requests", [])
    payment_logs = config.get("payment_logs", [])
    total_paid = config.get("finance", {}).get("total_paid", 0.0)
    ...
    finance_embed = discord.Embed(title="📊 Painel Financeiro", color=discord.Color.green())
    finance_embed.add_field(name="💰 Total vendido no período ADM", value=_format_brl(total_sales), inline=True)
    finance_embed.add_field(name="💸 Saldo pendente ADM", value=_format_brl(total_balance), inline=True)
    finance_embed.add_field(name="🏦 Total pago em saques", value=_format_brl(total_paid), inline=True)
    finance_embed.set_footer(text="Atualiza automaticamente")

    medals = ["🥇", "🥈", "🥉"]
    ranking = sorted(stats.items(), key=lambda x: (x[1].get("completed", 0), x[1].get("total_value", 0.0)), reverse=True)
    ranking_text = ""
    for i, (admin_id, data) in enumerate(ranking[:10]):
        medal = medals[i] if i < 3 else f"{i+1}."
        ranking_text += (
            f"{medal} <@{admin_id}>\n"
            f"📦 {data.get('completed', 0)} pedidos | "
            f"💰 {_format_brl(float(data.get('total_value', 0.0)))} | "
            f"💸 {_format_brl(float(data.get('balance', 0.0)))}\n\n"
        )
    ranking_embed = discord.Embed(
        title="🏆 Ranking de Atendimento",
        description=ranking_text or "Sem dados de atendimento ainda.",
        color=discord.Color.gold()
    )

    log_text = ""
    recent_withdraws = list(reversed(withdraws[-5:]))
    for w in recent_withdraws:
        status = "🟡 Pendente" if w.get("status") == "pending" else "🟢 Pago"
        log_text += f"ID {w.get('id')} | <@{w.get('user_id')}>\n💸 {_format_brl(float(w.get('valor', 0.0)))} | {status}\n\n"
    recent_payments = list(reversed(payment_logs[-5:]))
    if recent_payments:
        log_text += "### Últimos pagamentos\n"
        for p in recent_payments:
            log_text += f"<@{p.get('user_id')}> | {_format_brl(float(p.get('valor', 0.0)))} | {p.get('data')}\n"
    logs_embed = discord.Embed(
        title="📜 Saques e Pagamentos",
        description=log_text or "Sem registros ainda.",
        color=discord.Color.blurple()
    )
    return finance_embed, ranking_embed, logs_embed

async def update_status_embeds(bot):
    config = await db.get_config()
    dashboard = config.get("dashboard", {})
    finance_embed, ranking_embed, logs_embed = await _build_status_embeds()

    async def _upsert(channel_id, message_id, embed):
        if not channel_id:
            return message_id
        channel = bot.get_channel(channel_id)
        if not channel:
    ...
    dashboard["finance_message_id"] = await _upsert(dashboard.get("finance_channel"), dashboard.get("finance_message_id"), finance_embed)
    dashboard["ranking_message_id"] = await _upsert(dashboard.get("ranking_channel"), dashboard.get("ranking_message_id"), ranking_embed)
    dashboard["logs_message_id"] = await _upsert(dashboard.get("logs_channel"), dashboard.get("logs_message_id"), logs_embed)
    await db.update_config({"dashboard": dashboard})

async def register_admin_commission(order, admin_id: int):
    if not admin_id:
        return
    config = await db.get_config()
    stats = config.get("admin_stats", {})
    admin_key = str(admin_id)
    admin = stats.setdefault(admin_key, {
        "total_orders": 0,
        "total_value": 0.0,
        "completed": 0,
        "cancelled": 0,
    ...
    percent = float(config.get("admin_commission_percent", 5))
    order_value = float(order.get("valor", 0.0))
    admin["total_orders"] += 1
    admin["total_value"] += order_value
    admin["completed"] += 1
    admin["balance"] += order_value * (percent / 100.0)
    await db.update_config({"admin_stats": stats})
...
async def handle_purchase(interaction: discord.Interaction, product: dict, bot):
    ...
    try:
        ...
        order_data = {
            ...
            "produto_descricao": base_product.get('descricao', ''),
...
async def confirm_order_logic(order_id: int, bot, admin_id: int | None = None):
    try:
        order = await db.get_order_by_id(order_id)
        if not order or order['status'] != OrderStatus.PENDING.value: return
        update_payload = {"status": OrderStatus.CONFIRMED.value, "confirmed_at": datetime.now(timezone.utc).isoformat()}
        if admin_id:
            update_payload["admin_id"] = admin_id
        await db.update_order(order_id, update_payload)
        order = await db.get_order_by_id(order_id)
        if admin_id:
            await register_admin_commission(order, admin_id)
            await update_status_embeds(bot)
...
async def cancel_order_logic(order_id: int, reason: str, bot, admin_id: int | None = None):
    order = await db.get_order_by_id(order_id)
    if not order or order['status'] != OrderStatus.PENDING.value: return
    ...
    if admin_id:
        config = await db.get_config()
        stats = config.get("admin_stats", {})
        adm = stats.setdefault(str(admin_id), {"total_orders": 0, "total_value": 0.0, "completed": 0, "cancelled": 0, "total_time": 0.0, "balance": 0.0})
        adm["cancelled"] += 1
        await db.update_config({"admin_stats": stats})
        await update_status_embeds(bot)
...
if __name__ == "__main__":
    try:
        ...
        @bot.tree.command(name="saldoadm", description="Mostra seu saldo e desempenho como atendente.")
        async def saldoadm(interaction: discord.Interaction):
            config = await db.get_config()
            stats = config.get("admin_stats", {})
            data = stats.get(str(interaction.user.id), {"total_orders": 0, "total_value": 0.0, "completed": 0, "cancelled": 0, "balance": 0.0})
            embed = discord.Embed(title="💸 Seu saldo ADM", color=discord.Color.blue())
            embed.add_field(name="Saldo", value=_format_brl(float(data.get("balance", 0.0))), inline=True)
            embed.add_field(name="Vendido no período", value=_format_brl(float(data.get("total_value", 0.0))), inline=True)
            embed.add_field(name="Concluídos", value=str(data.get("completed", 0)), inline=True)
            await interaction.response.send_message(embed=embed, ephemeral=True)

        @bot.tree.command(name="solicitarsaque", description="Solicita saque do seu saldo ADM.")
        @app_commands.describe(valor="Valor do saque em reais")
        async def solicitarsaque(interaction: discord.Interaction, valor: float):
            config = await db.get_config()
            stats = config.get("admin_stats", {})
            data = stats.get(str(interaction.user.id))
            if not data or float(data.get("balance", 0.0)) < valor:
                return await interaction.response.send_message("❌ Saldo insuficiente.", ephemeral=True)
            reqs = config.get("withdraw_requests", [])
            ...
            await db.update_config({"withdraw_requests": reqs})
            await interaction.response.send_message(f"✅ Saque #{new_id} solicitado com sucesso.", ephemeral=True)
            await update_status_embeds(bot)

        @bot.tree.command(name="aprovarsaque", description="Aprova um saque pendente pelo ID.")
        @app_commands.describe(saque_id="ID da solicitação de saque")
        async def aprovarsaque(interaction: discord.Interaction, saque_id: int):
            config = await db.get_config()
            role_id = config.get("cargo_aprovador_saque")
            if not role_id or not any(r.id == role_id for r in interaction.user.roles):
                return await interaction.response.send_message("❌ Você não tem permissão para aprovar saques.", ephemeral=True)
            reqs = config.get("withdraw_requests", [])
            target = None
            for r in reqs:
                if int(r.get("id", 0)) == saque_id and r.get("status") == "pending":
                    target = r
                    break
            if not target:
                return await interaction.response.send_message("❌ Saque pendente não encontrado.", ephemeral=True)
            stats = config.get("admin_stats", {})
            adm = stats.setdefault(str(target["user_id"]), {"total_orders": 0, "total_value": 0.0, "completed": 0, "cancelled": 0, "total_time": 0.0, "balance": 0.0})
            vendas_resetadas = float(adm.get("total_value", 0.0))
            adm["balance"] = 0.0
            adm["total_value"] = 0.0
            target["status"] = "paid"
            target["approved_at"] = datetime.now(timezone.utc).isoformat()
            logs = config.get("payment_logs", [])
            logs.append({
                "user_id": target["user_id"],
                "valor": float(target["valor"]),
                "vendas_resetadas": vendas_resetadas,
                "data": datetime.now().strftime("%Y-%m-%d %H:%M"),
                "aprovado_por": interaction.user.id
            })
            finance = config.get("finance", {"total_paid": 0.0})
            finance["total_paid"] = float(finance.get("total_paid", 0.0)) + float(target["valor"])
            await db.update_config({"admin_stats": stats, "withdraw_requests": reqs, "payment_logs": logs, "finance": finance})
            await interaction.response.send_message(f"✅ Saque #{saque_id} aprovado.", ephemeral=True)
            await update_status_embeds(bot)

        @bot.tree.command(name="rankingadms", description="Mostra o ranking de atendimento dos ADMs.")
        async def rankingadms(interaction: discord.Interaction):
            config = await db.get_config()
            _, ranking_embed, _ = await _build_status_embeds()
            await interaction.response.send_message(embed=ranking_embed, ephemeral=True)

        @bot.tree.command(name="atualizarpaineis", description="Força atualização dos painéis de status.")
