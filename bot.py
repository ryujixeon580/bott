...
class InMemoryDatabase:
    def __init__(self, filename):
        ...
        self._db_cache = {
            "config": {
                ...
                "pix_cidade": "BRASIL", "quantidade_por_categoria": {},
                "cargo_atendimento_id": None, "admin_commission_percent": 5,
                "cargo_aprovador_saque": None,
                "attendance_config": {"channel_id": None, "message_id": None, "role_id": None},
                "dashboard": {"finance_channel": None, "ranking_channel": None, "logs_channel": None, "finance_message_id": None, "ranking_message_id": None, "logs_message_id": None},
                "finance": {"total_paid": 0},
                "admin_stats": {}, "withdraw_requests": [], "payment_logs": []
            },
            "products": [], 
            "orders": [], 
            "active_catalogs": [],
    ...
    async def get_config(self):
        async with self._lock:
            ...
            cfg.setdefault("attendance_config", {"channel_id": None, "message_id": None, "role_id": None})
            ...
            self._db_cache.setdefault("attendance", {})
...
class AdminActionsView(discord.ui.View):
    ...
    async def claim_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        try: await interaction.edit_original_response(content="✅ **Pedido reivindicado com sucesso.**", view=self)
...
class AttendanceView(discord.ui.View):
    ...
    async def _check(self, interaction):
        allowed = await check_staff_permission(interaction, self.bot)
        if not allowed:
    ...
    async def start_work(self, interaction: discord.Interaction, button: discord.ui.Button):
        ...
        att = db._db_cache.setdefault("attendance", {}).setdefault(str(interaction.user.id), {})
        att.update({"status":"active","clock_in":datetime.now(timezone.utc).isoformat(),"clock_out":None,"period_orders":0,"period_value":0,"period_commission":0})
    ...
    async def pause_work(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not await self._check(interaction): return
        att = db._db_cache.setdefault("attendance", {}).setdefault(str(interaction.user.id), {})
        att["status"] = "paused"
    ...
    async def stop_work(self, interaction: discord.Interaction, button: discord.ui.Button):
        if not await self._check(interaction): return
        att = db._db_cache.setdefault("attendance", {}).setdefault(str(interaction.user.id), {})
        att["status"] = "offline"
        att["clock_out"] = datetime.now(timezone.utc).isoformat()
        await interaction.response.send_message("🔴 Turno encerrado.", ephemeral=True)
        await update_attendance_panel(self.bot)

class StaffRoleSetupModal(discord.ui.Modal, title="Cargo de Atendimento"):
    def __init__(self, bot_instance, current_id=None):
        super().__init__()
        self.bot = bot_instance
        self.role_id = discord.ui.TextInput(label="ID do cargo de atendimento", default=str(current_id or ""), required=False)
        self.add_item(self.role_id)
    async def on_submit(self, interaction: discord.Interaction):
        value = self.role_id.value.strip() or None
        await db.update_config({"cargo_atendimento_id": int(value) if value else None})
        await interaction.response.send_message("✅ Cargo de atendimento atualizado.", ephemeral=True)

class WithdrawApproverSetupModal(discord.ui.Modal, title="Cargo que aprova saque"):
    def __init__(self, bot_instance, current_id=None):
        super().__init__()
        self.bot = bot_instance
        self.role_id = discord.ui.TextInput(label="ID do cargo aprovador", default=str(current_id or ""), required=False)
        self.add_item(self.role_id)
    async def on_submit(self, interaction: discord.Interaction):
        value = self.role_id.value.strip() or None
        await db.update_config({"cargo_aprovador_saque": int(value) if value else None})
        await interaction.response.send_message("✅ Cargo aprovador atualizado.", ephemeral=True)

class CommissionSetupModal(discord.ui.Modal, title="Comissão ADM"):
    def __init__(self, bot_instance, current_percent=5):
        super().__init__()
        self.bot = bot_instance
        self.percent = discord.ui.TextInput(label="Percentual (%)", default=str(current_percent))
    ...
    async def on_submit(self, interaction: discord.Interaction):
        try:
            ...
            await interaction.response.send_message("✅ Comissão atualizada.", ephemeral=True)
        except:
            await interaction.response.send_message("❌ Valor inválido.", ephemeral=True)

class AttendancePanelConfigModal(discord.ui.Modal, title="Painel de Bate-Ponto"):
    def __init__(self, bot_instance, cfg):
        super().__init__()
        self.bot = bot_instance
        self.channel_id = discord.ui.TextInput(label="ID do canal do bate-ponto", default=str(cfg.get("channel_id") or ""), required=False)
        self.add_item(self.channel_id)
    async def on_submit(self, interaction: discord.Interaction):
        config = await db.get_config()
        att_cfg = config.get("attendance_config", {})
        value = self.channel_id.value.strip() or None
        await db.update_config({"attendance_config": {**att_cfg, "channel_id": int(value) if value else None}})
        await interaction.response.send_message("✅ Canal do bate-ponto atualizado.", ephemeral=True)
        await update_attendance_panel(self.bot)

class DashboardPanelConfigModal(discord.ui.Modal, title="Painéis de Status"):
    def __init__(self, bot_instance, cfg):
        super().__init__()
        self.bot = bot_instance
        self.finance = discord.ui.TextInput(label="ID canal financeiro", default=str(cfg.get("finance_channel") or ""), required=False)
        self.ranking = discord.ui.TextInput(label="ID canal ranking", default=str(cfg.get("ranking_channel") or ""), required=False)
        self.add_item(self.finance); self.add_item(self.ranking)
    async def on_submit(self, interaction: discord.Interaction):
        config = await db.get_config()
        dash = config.get("dashboard", {})
        await db.update_config({"dashboard": {**dash, "finance_channel": int(self.finance.value) if self.finance.value.strip() else None, "ranking_channel": int(self.ranking.value) if self.ranking.value.strip() else None}})
        await interaction.response.send_message("✅ Painéis atualizados.", ephemeral=True)
        await update_status_dashboards(self.bot)
...
class ConfigSelectionView(discord.ui.View):
    ...
    async def staff_role(self, interaction: discord.Interaction, button: discord.ui.Button):
        config_data = await db.get_config()
        await interaction.response.send_modal(StaffRoleSetupModal(self.bot, config_data.get("cargo_atendimento_id")))
    @discord.ui.button(label="💸 Comissão ADM", style=discord.ButtonStyle.secondary)
    async def commission_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        config_data = await db.get_config()
        await interaction.response.send_modal(CommissionSetupModal(self.bot, config_data.get("admin_commission_percent", 5)))
    @discord.ui.button(label="🛡️ Cargo aprova saque", style=discord.ButtonStyle.secondary)
    async def withdraw_role_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        config_data = await db.get_config()
        await interaction.response.send_modal(WithdrawApproverSetupModal(self.bot, config_data.get("cargo_aprovador_saque")))
    @discord.ui.button(label="🕒 Bate-Ponto", style=discord.ButtonStyle.secondary)
    async def attendance_panel_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        config_data = await db.get_config()
        await interaction.response.send_modal(AttendancePanelConfigModal(self.bot, config_data.get("attendance_config", {})))
    @discord.ui.button(label="📊 Painéis de Status", style=discord.ButtonStyle.secondary)
    async def status_panel_btn(self, interaction: discord.Interaction, button: discord.ui.Button):
        config_data = await db.get_config()
        await interaction.response.send_modal(DashboardPanelConfigModal(self.bot, config_data.get("dashboard", {})))
...
async def confirm_order_logic(order_id: int, bot):
    try:
        ...
        order = await db.get_order_by_id(order_id)
        await register_admin_sale(order, bot)
...
if __name__ == "__main__":
    try:
        ...
        async def saldoadm(interaction: discord.Interaction):
            ...
            await interaction.response.send_message(f"💸 Seu saldo atual: {_fmt_money(data.get('balance', 0))}", ephemeral=True)

        @app_commands.describe(valor="Valor do saque")
        @bot.tree.command(name="solicitarsaque", description="Solicita um saque do seu saldo.")
        async def solicitarsaque(interaction: discord.Interaction, valor: float):
            ...
            stats = config.get("admin_stats", {})
            data = stats.get(str(interaction.user.id), {})
            if valor <= 0 or valor > float(data.get("balance", 0)):
            ...
            req_id = (max([r.get("id",0) for r in reqs]) + 1) if reqs else 1
            reqs.append({"id": req_id, "user_id": interaction.user.id, "valor": valor, "status": "pending", "created_at": datetime.now(timezone.utc).isoformat()})
            await db.update_config({"withdraw_requests": reqs})
            await interaction.response.send_message(f"✅ Solicitação criada com ID #{req_id}.", ephemeral=True)

        @app_commands.describe(id="ID da solicitação")
        @bot.tree.command(name="aprovarsaque", description="Aprova uma solicitação de saque.")
        async def aprovarsaque(interaction: discord.Interaction, id: int):
            config = await db.get_config()
            approver_role = config.get("cargo_aprovador_saque")
            user_roles = [r.id for r in interaction.user.roles]
            if not (interaction.user.guild_permissions.administrator or (approver_role and approver_role in user_roles) or await check_manager_permission(interaction, bot)):
                return await interaction.response.send_message("❌ Sem permissão.", ephemeral=True)
            reqs = config.get("withdraw_requests", [])
            target = next((r for r in reqs if r.get("id")==id and r.get("status")=="pending"), None)
            if not target:
                return await interaction.response.send_message("❌ Solicitação não encontrada.", ephemeral=True)
            stats = config.get("admin_stats", {})
            adm = stats.setdefault(str(target["user_id"]), {"total_orders":0,"total_value":0,"completed":0,"cancelled":0,"total_time":0,"balance":0})
            vendas_resetadas = adm.get("total_value", 0)
            adm["balance"] = 0
            adm["total_value"] = 0
            target["status"] = "paid"
            logs = config.get("payment_logs", [])
            logs.append({"user_id": target["user_id"], "valor": target["valor"], "vendas_resetadas": vendas_resetadas, "data": datetime.now(timezone.utc).isoformat(), "aprovado_por": interaction.user.id})
            finance = config.get("finance", {"total_paid": 0})
            finance["total_paid"] = finance.get("total_paid", 0) + float(target["valor"])
            await db.update_config({"withdraw_requests": reqs, "admin_stats": stats, "payment_logs": logs, "finance": finance})
            await update_status_dashboards(bot)
            await interaction.response.send_message(f"✅ Saque #{id} aprovado.", ephemeral=True)

        @bot.tree.command(name="rankingadms", description="Mostra o ranking de atendimento.")
        async def rankingadms(interaction: discord.Interaction):
            if not await check_staff_permission(interaction, bot):
                return await interaction.response.send_message(view=NoPermissionView(), ephemeral=True)
            config = await db.get_config()
            stats = config.get("admin_stats", {})
            ranking = sorted(stats.items(), key=lambda x: x[1].get("completed",0), reverse=True)
            desc = ""
            for i, (admin_id, data) in enumerate(ranking[:10], 1):
                desc += f"{i}. <@{admin_id}> | 📦 {data.get('completed',0)} | 💰 {_fmt_money(data.get('total_value',0))} | 💸 {_fmt_money(data.get('balance',0))}\n"
            embed = discord.Embed(title="🏆 Ranking Atendimento", description=desc or "Sem dados.", color=discord.Color.gold())
            await interaction.response.send_message(embed=embed, ephemeral=True)

        @bot.tree.command(name="atualizarpaineis", description="Atualiza os painéis de status e bate-ponto.")
        async def atualizarpaineis(interaction: discord.Interaction):
            if not await check_manager_permission(interaction, bot):
                return await interaction.response.send_message(view=NoPermissionView(), ephemeral=True)
            await interaction.response.defer(ephemeral=True)
            await update_status_dashboards(bot)
